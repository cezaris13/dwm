...
result="$(cat links_and_titles.txt | grep -o ">.*$" | grep -B1 "Chapter" | sed "s/> *//g" | grep -vA1 "Chapter" |awk 'length >4'|  awk 'NR%2{printf "%s ",$0;next;}1' |  dmenu -i -l 20 -p "new manga")"
