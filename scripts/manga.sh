...
result="$(echo $result | sed "s/[Cc]hapter.*//g" | sed "s/[A-Z]/\L&/g" | sed "s/[Yy]ou're//g;s/[Tt]he//g;s/[']s//g;s/ /-/g;s/[!,;']//g;s/\-\+/-/g" | grep -o "[A-Za-z].*[A-Za-z]"  ) " #optimise later
link="$(cat index.html | sed "s/>/>\n/g" | grep -o " href=\"http.*\"" | grep "[Cc]hapter" | sed "s/\" title.*$//g;s/\/\".\+$//g;s/href=\"//g;s/\/\"$//g" | grep -m1 $result )"
# echo $link
echo ${#result}
echo ${#link}
