...
emoji="$(cat $HOME/.config/dwm/scripts/emoji.txt |  rofi  -modi run -location 1 -width 100 \
		 ...
		 -kb-row-select "Tab" -kb-row-tab "" -dmenu -i -p ""  -theme-str 'entry { placeholder: ""; } inputbar { children: [prompt, textbox-prompt-colon, entry];}' -theme-str ' inputbar { children: [ prompt, textbox-prompt-colon, entry, case-indicator ];}    textbox-prompt-colon { text-color: inherit; expand: false; margin: 0 0.3em 0em 0em; str: ":"; }' -show | awk '{print $1}' | tr -d '\n' )"
