...
createmon(void)
{
	...
	m->mfact = mfact;
	m->nmaster = nmaster;// change this if i want 0 nmaster on 2 screen
	m->showbar = showbar;
	m->topbar = topbar;
	m->gappx = gappx;
	m->lt[0] = &layouts[0];
...
deck(Monitor *m) {
	...
	for(i = 0, my = gappx, c = nexttiled(m->cl->clients, m); c; c = nexttiled(c->next, m), i++)
		if(i < m->nmaster) {
...
saveSession(void)
{
	...
	for (Client *c = selmon->cl->clients; c != NULL; c = c->next) { // get all the clients with their tags and write them to the file
		fprintf(fw, "%lu %u\n", c->win, c->tags);
	}
	fclose(fw);
}
...
restoreSession(void)
{
	...
	while (fscanf(fr, "%[^\n] ", str) != EOF) { // read file till the end
		...
		for (Client *c = selmon->cl->clients; c ; c = c->next) { // add tags to every window by winId
			if (c->win == winId) {
				c->tags = tagsForWin;
				break;
			}
	...
	for (Client *c = selmon->cl->clients; c ; c = c->next) { // refocus on windows
		focus(c);
		restack(c->mon);
	}
...
setgaps(const Arg *arg)
{
	if ((arg->i == 0) || (selmon->gappx + arg->i < 0))
		selmon->gappx = 0;
	else if ((arg->i == gappx) || (selmon->gappx + arg->i < 0))
		selmon->gappx = gappx;
	else
		selmon->gappx += arg->i;
	arrange(selmon);
...
tile(Monitor *m)
{
	...
	Client *c;
    m->nmaster=(m->mw<m->mh?0:m->nmaster);//this line makes vertical monitor have 0 clients in master area
	for (n = 0, c = nexttiled(m->cl->clients, m); c; c = nexttiled(c->next, m), n++);
	if (n == 0)
		return;
...
toggleview(const Arg *arg)
{
	...
	if (newtagset) {
		...
		unsigned int newtagset = selmon->tagset[selmon->seltags] ^ (arg->ui & TAGMASK);//looks like this one is useless
